...
void WHITE_LED(void const * argument);
...
int main(void)
{
  ...
  osThreadDef(Blink1, GREEN_LED, osPriorityNormal, 0, 128);
  Blink1Handle = osThreadCreate(osThread(Blink1), NULL);

  /* definition and creation of BLINK_2 */
  osThreadDef(BLINK_2, WHITE_LED, osPriorityNormal, 0, 128);
  BLINK_2Handle = osThreadCreate(osThread(BLINK_2), NULL);

  /* definition and creation of Servo_Control */
  osThreadDef(Servo_Control, Servo, osPriorityNormal, 0, 128);
...
void Servo(void const * argument)
{
  ...
  for(;;)
  {
	for(int i=1000;i<2500;i=i+10)
	{
		__HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_2,i);
		osDelay(10);
	}
    osDelay(1);
    for(int i=2500;i>1000;i=i-10)
    {
    		__HAL_TIM_SET_COMPARE(&htim2,TIM_CHANNEL_2,i);
    		osDelay(10);
    	}
